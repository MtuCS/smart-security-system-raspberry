...
class FaceEngine:
    def __init__(self, det_size=(416, 416), model_name="buffalo_s", cache_path: Optional[str | Path] = None):
        self.app = FaceAnalysis(name=model_name)
        self.app.prepare(ctx_id=-1, det_size=det_size)
        self.db = FaceDatabase()
        self.cache_path = Path(cache_path) if cache_path else Path(getattr(config, "FACE_DB_CACHE_PATH", config.FACE_DB_CACHE_FILE))
    ...
    def _manifest_hash(self, manifest: List[Tuple[str, int, float]]) -> str:
        return hashlib.sha256(repr(manifest).encode("utf-8")).hexdigest()
    ...
    def _save_cache(self, manifest_hash: str):
        try:
            ...
            print(f"[INFO] Saved face DB cache: {self.cache_path}")
        except Exception as e:
    ...
    def build_database(self, db_dir: str):
        ...
        if getattr(config, "ENABLE_FACE_DB_CACHE", True) and self._load_cache(manifest_hash):
            return
        ...
        if getattr(config, "ENABLE_FACE_DB_CACHE", True):
            self._save_cache(manifest_hash)
    ...
    def infer(self, frame: np.ndarray, threshold: float = 0.45) -> List[Dict[str, Any]]:
        ...
        for face in faces:
            ...
            results.append({"bbox": bbox, "name": name, "distance": dist})
        return results
